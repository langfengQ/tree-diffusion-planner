...
def eval_episode(model, env, dataset, traj_agg_tree, savepath_i, seed, is_render=True):
    ...
    for i in range(3):
        ...
        samples = samples_orig = trainer.ema_model.guided_conditional_sample(model, args.batch_size, conditions, cond_idx, stack, place)
...
if args.use_tree:
    savepath = f'logs/{env_name}/plans_rearrangment_weighted{weighted}_{H}_{T}_bs{args.batch_size}_tree_lambda{args.tree_lambda}_th{args.one_minus_alpha}'
else:
    savepath = f'logs/{env_name}/plans_rearrangment_weighted{weighted}_{H}_{T}_bs{args.batch_size}_vanilla'
...
for i in tqdm(range(100)):
    ...
    reward, rearrange_goal = eval_episode(model, env, dataset, traj_agg_tree, savepath_i=savepath_i, seed=seed)
    ...
    json_data = {'score': reward, 'return': reward, "rearrange_goal": rearrange_goal}
